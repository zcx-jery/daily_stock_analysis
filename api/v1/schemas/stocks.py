...
class MomentumScreenerResult(BaseModel):
    ...
    amount: Optional[float] = Field(None, description="T 日成交额，用于放量滞涨审计")
    turnover_rate_f: Optional[float] = Field(None, description="T 日自由流通换手率，用于量能竭尽 R5 审计")
    volume_expand_5: Optional[float] = Field(None, description="T 日成交量相对近 5 日均量的放大倍数，缺少量字段时降级用成交额")
    amount_10d_high: Optional[bool] = Field(None, description="T 日成交额是否创近 10 日新高")
    price_gain_shrinking: Optional[bool] = Field(None, description="T 日涨幅是否较近 3 日强势峰值收缩")
    close_position: Optional[float] = Field(None, description="T 日收盘在日内振幅中的位置，0=最低，1=最高")
    upper_shadow_ratio: Optional[float] = Field(None, description="T 日上影线占日内振幅比例")
...
class MomentumDecisionPortfolioSlot(BaseModel):
    ...
    v13_ladder_position: Optional[Dict[str, Any]] = Field(None, description="V1.3 涨停梯队 / 空间高度画像")
    ...
    risk_stack_count: Optional[int] = Field(None, description="风险堆叠点数（R5 计 2 点）")
    risk_stack_veto: Optional[bool] = Field(None, description="是否因风险堆叠被硬阻断")
    raw_alpha_shield: Optional[Dict[str, Any]] = Field(None, description="Raw Top3 Alpha Shield 保护/剔除解释")
    continuation_alpha: Optional[Dict[str, Any]] = Field(None, description="Continuation Alpha / Deep Challenger 重排解释")
    continuation_rank: Optional[int] = Field(None, description="全候选池延续分排名")
    t1_support_probability: Optional[float] = Field(None, description="T+1 support probability")
    support_inertia_coefficient: Optional[float] = Field(None, description="Support inertia coefficient")
    support_adjusted_continuation_score: Optional[float] = Field(None, description="Support-adjusted continuation score")
    support_components: Optional[Dict[str, Any]] = Field(None, description="T+1 support probability components")
...
class MomentumBacktestSummary(BaseModel):
    ...
    anchor_supremacy_report: Dict[str, Any] = Field(default_factory=dict, description="V1.3 锚点至上主仓与 Raw #1 的区间对照")
    ...
    data_integrity_warnings: Dict[str, Any] = Field(default_factory=dict, description="回测摘要读取降级与坏页跳过信息")
...
class MomentumBacktestBenchmarkItem(BaseModel):
    ...
    positive_t2_rate_pct: Optional[float] = Field(None, description="可交易合格率：非 T+1 一字不可买、T+1 不深低开且收阳、T+2 滑点调整退出价至少给出 2% 利润缓冲(%)")
...
class MomentumBacktestDailyListResponse(BaseModel):
    ...
    data_integrity_warnings: Dict[str, Any] = Field(default_factory=dict, description="回测日级列表读取降级与坏页跳过信息")
...
class MomentumBacktestOutcomeItem(BaseModel):
    ...
    t2_close_price: Optional[float] = Field(None, description="T+2 收盘价")
    t2_slippage_adjusted_exit_price: Optional[float] = Field(None, description="T+2 滑点调整退出价")
    ...
    tradable_profit_window_pass: Optional[bool] = Field(None, description="T+2 滑点调整退出价是否给出 2% 利润缓冲")
...
class MomentumBacktestDecisionDetailItem(BaseModel):
    ...
    continuation_score: Optional[float] = Field(None, description="延续分")
    continuation_rank: Optional[int] = Field(None, description="全候选池延续分排名")
    continuation_alpha: Optional[Dict[str, Any]] = Field(None, description="Continuation Alpha / 深度挑战者解释")
    t1_support_probability: Optional[float] = Field(None, description="T+1 support probability")
    support_inertia_coefficient: Optional[float] = Field(None, description="Support inertia coefficient")
    support_adjusted_continuation_score: Optional[float] = Field(None, description="Support-adjusted continuation score")
...
class MomentumBacktestIssueListResponse(BaseModel):
    ...
    data_integrity_warnings: Dict[str, Any] = Field(default_factory=dict, description="回测问题清单读取降级与坏页跳过信息")
