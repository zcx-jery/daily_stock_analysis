...
flowchart TD
    ...
    G --> H["对比候选池 Top10、正式排序前排代理（当前为原始排序 Top3）和主仓基准"]
    H --> I["输出通过 / 观察 / 失败诊断"]
```
...
| 原始排序 Top3 | 判断收口层是否过度偏离正式排序前排 | 官方 Top3 是否明显弱于正式排序前排代理 |
| 主仓基准 | 判断主仓锚点是否稳定承担组合核心职责 | 主仓是否长期弱于官方 Top3 均值 |

不能只看官方 Top3 单独胜率。若官方 Top3 胜率不错，但不如候选池 Top10，说明整条官方链路没有把候选池机会有效收口成可执行组合；若主要弱于原始排序 Top3 或主仓基准，更像是收口层、主仓锚点或总闸门过度偏移，而不再简单归因为“二次决策没有创造增量价值”。
...
V1.3 起，这一门槛不再定义为“二次决策必须额外创造 Alpha”。
主排序层已经承担“谁更强”的正式判断；二次决策只负责把正式排序收口为 `主仓 / 次仓 / 观察仓`，并补上动作闸门、执行提示和落选解释。
...
| 相对正式排序前排代理 | 当前实现暂以 `原始排序 Top3` 作为前排代理；官方 Top3 不得长期低于该代理超过 `3` 个百分点 | 判断收口层是否过度扭曲正式排序前排，而不是要求二次决策额外创造 Alpha |
| 相对主仓基准 | 主仓合格率不得长期低于官方 Top3 均值超过 `5` 个百分点 | 判断主仓锚点是否稳定承担组合核心职责 |

如果官方 Top3 绝对胜率达标，但相对基准仍偏弱，结论应写成“主排序或收口层仍待校准”，而不是简单写成“二次决策没有增益”。

如果官方 Top3 主要弱于候选池 Top10，更优先怀疑候选池质量或主排序；如果主要弱于正式排序前排代理或主仓基准，更优先怀疑收口层、主仓锚点或总闸门。
...
| `失败` | 官方 Top3 合格率 `<50%`，或明显弱于候选池 / 正式排序前排代理 / 主仓基准 | 必须回到入口、主排序、收口层或总闸门规则重新校准 |
...
3. 收口层：官方 Top3 / 主仓 / 次仓 / 观察仓是否过度偏离正式排序前排；当前实现可先用 `原始排序 Top3` 作为前排代理做诊断。
...
| 指标 | 官方 Top3 | 候选池 Top10 | 正式排序前排代理（当前为原始排序 Top3） | 主仓 |
| --- | --- | --- | --- | --- |
| 短线延续合格率 |  |  |  |  |
| 平均 T+2 利润窗口 |  |  |  |  |
| 平均最大回撤 |  |  |  |  |
...
- V1.3 起，`原始排序 Top3` 在官方验收里更适合作为“正式排序前排代理”使用，用于检查收口层是否过度偏离前排，而不再默认解释为“二次决策必须创造增量价值”。
- 若后续回测新增 `official_score_top3` 或等价的正式排序前排独立基准，应优先用它替代 `原始排序 Top3` 作为收口层验收代理。
