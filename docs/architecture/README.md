...
- `V1.3` 回测验收以 60 交易日为标准窗口，主胜率统一使用 `可交易合格率`，弱延续合格率只作为方向辅助参考
- `V1.3` 可交易合格主标签要求：`T+1` 非一字不可买、`T+1 open >= 0.99 * T0 close`、`T+1 close > T+1 open`，且 `(T+2 high + T+2 close) / 2 >= 1.02 * T+1 close`
- `V1.3` Alpha 审计必须同时比较官方 Top3、Raw Momentum Top3 与全候选池基准；官方 Top3 相对全池不足 `15pct` 时标记 `ALPHA_EROSION_DETECTED`
- `V1.3` 总闸门采用 `市场环境 × 当日机会质量` 主矩阵，并由 `20日进攻许可` 做动作封顶；`60日主线可信度` 只做结构提示，最近防守成功率可驱动 Adaptive Gate 收紧主线阈值
- `V1.3` 基于 6000 积分升级主线增强能力，接入题材成分、涨停炸板、涨跌停价、热榜、Risk Stack、Mainline Intensity 与 Elite Protocol 梯队画像；市场最高连板空间龙可豁免主线不足 R4
- `V1.3` AI 点评定位为短线交易逻辑审计员，必须解释 `[Core Logic] / [Risk Audit] / [Execution Guard]`，并在 `T+1` 未突破开盘后 30 分钟高点时提示 `Reduce Position`
