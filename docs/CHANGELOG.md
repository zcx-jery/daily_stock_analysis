...
- [??] ????? AI ?????????? `official_score`???? Candidate Top10 ????????????????
- [??] ??????????? `official_score` ?????????????/??/???????????????????? `_decision_score` ???????????
- [??] ?????????? `rank_score` ???Aggressive ???????????????????????????????????????????????????????
- [改进] 二次决策开始优先使用 `official_score` 作为官方基准分输入，并在组合卡、候选诊断和落选说明里同步暴露官方总分，减少旧 `rank_score` 对收口层的主导。
- [改进] 强势筛选 Standard 个股列表固定按 `official_score`（官方总分）展示并移除普通用户多排序入口，延续分/弹性分/风险分/可买分继续保留为解释层。
- [??] `official_score` ????????????????????V1.3 ????????????????????????????????????
- [改进] 强势筛选主评分链开始显式产出 `official_score` 并统一通过正式排序收口逻辑生成排名，缓存回放也会补齐该字段，为“全候选正式重评分 -> 官方总分”主链清场。
- [改进] 强势筛选正式评分继续收口 full/light 边界：全候选正式画像与正式重评分阶段使用更明确的运行文案，官方回测显式改走 `full` 真值链路并在请求参数中保留 `truth_mode=full`，避免沿用旧的 `light` 过渡口径。
- [文档] 继续细化“全候选主排序引擎重构”实施方案：补充 `官方总分` 正式产出链、全候选统一画像与统一评分的技术落点，以及第一阶段实施任务清单。
- [文档] 整理强势筛选后续需求池清单，按“已冻结待实现 / 待讨论 / 后置”收口全候选主排序、唯一官方总分、二次决策收口层、回测新口径验证和停用恢复链路的推进顺序。
- [??] ???????????????????????????????????? `official_score` ???????????????????????????????????
- [文档] 冻结二次决策改造方向：二次决策从“重排层”收缩为“组合、动作、解释收口层”，不再用解释因子推翻正式主排序。
- [文档] 记录“初筛后全候选按 6000 积分新画像统一重评分、替换旧 V1 正式排序逻辑”的需求冻结项。
...
- [新功能] 强势筛选新增任务化 run 后端骨架，支持创建任务、进度轮询、结果读取与取消，并为真实性优先的完整 V1.3 链路预留 full truth_mode。
- [文档] 明确强势筛选真实性优先执行模式：参照回测改为任务化筛选、阶段进度刷新与分层缓存复用。
- [修复] 强势筛选正式评分新增轻量 V1.3 screening context，并把同步增强样本收紧到前 12 名，避免逐股筹码和同花顺成分查询拖到网关超时。
- [修复] 强势筛选正式评分的 V1.3 个股级增强只覆盖前排候选，避免大候选池日逐股补全资金流、筹码和题材成分时触发网关超时。
...
- [改进] 强势筛选二次决策新增 V1.3 影子分诊断字段，先观察真实题材、资金、涨停结构、买点和筹码风险分项，不改变官方排序。
...
- [改进] 强势筛选二次决策将 V1.3 “主线雷达”改为“题材强弱诊断”，接入同花顺 `ths_index` 把 `.TI` 概念代码翻译为中文题材名，并用候选股、前 10 名、涨停、炸板和热榜排名解释短线情绪来源。
- [改进] 强势筛选 V1.3 题材识别新增父级资金主线归并层，将锂电池、固态电池、电解液、隔膜、锂矿等碎片概念聚合为“电池”等可复盘的大主线，并在页面展示覆盖子题材。
- [改进] 强势筛选二次决策新增 T 日可见的 `t1_direction_risk_adjustment`，对高延伸、高延续和高排序共振增加过热饱和惩罚，并在 V1 回测摘要、基准和问题清单中拆出 T+1 方向 / T+2 延续诊断，帮助定位短线延续失败瓶颈。
- [改进] 强势筛选 V1.3 主线增强在完整 ranked_results 回放时只对前排样本拉取同花顺概念上下文，避免 Standard 60 日回测首日因全量候选逐股请求而长时间停在 0%。
...
- [改进] 强势筛选 V1.3 补齐二次决策 API Schema 与前端 TypeScript 类型，新增字段均保持可选以兼容旧响应。
- [改进] 强势筛选二次决策主链路接入 V1.3 主线雷达与短线情绪数据，组合排序会优先吸收同花顺概念映射、主线强弱分和数据源降级状态，同时保留旧规则回退。
- [改进] 强势筛选 V1.3 数据聚合服务新增短线情绪评分底座，汇总涨停强度、炸板风险、主线清晰度、热榜集中度和昨日强势反馈。
- [改进] 强势筛选 V1.3 数据聚合服务新增主线雷达评分，可基于候选池密度、涨停强度、炸板风险、热榜集中度和昨日反馈输出 Top 主线证据。
- [改进] 强势筛选 V1.3 新增数据聚合服务，支持主线上下文、回测上下文、盘中快照辅助、资源级缓存和数据源降级状态汇总。
- [改进] 强势筛选 V1.3 数据底座新增 Tushare `stk_limit`、`limit_list_d`、`ths_member`、`ths_hot` 适配封装，统一输出降级状态、数据时间和 JSON 安全字段。
- [文档] 新增强势筛选 V1.3 主线增强版产品设计、技术开发文档和开发任务清单，并同步 SPEC、PRD、产品方案、V1 产品原则与架构索引到 6000 积分数据权限下的主线识别、短线情绪、盘中快照辅助和回测诊断边界。
- [文档] 收口强势筛选 20/60 新定位：`20日进攻温度` 负责短期风险温度，`60日结构信任度` 负责长期结构可信度，并同步 SPEC、产品原则与新总闸门草案。
- [改进] 新增 `scripts/start_momentum_backtest_task.py`，可从命令行直接发起 V1 强势筛选回测任务并返回 `run_id`，便于手动启动长区间 strict 20/60 研究回测。
- [改进] 校准强势筛选 `20 日进攻许可`：在保留命中率硬门槛外，新增“高利润窗口 + 回撤可控”的恢复通道，避免近期强市场里历史窗长期恒定 `paused` 并把明显可做日误压成 `仅观察 / 今日不做`。
- [改进] V1 强势筛选回测新增严格 `20/60` 历史窗模式：创建回测时可切换 `strict_strategy_health=true`，后端会逐日强制等待历史窗进入 `final/failed`，并在 run/summary 返回 `strategy_health_mode`、验证状态分布、20日进攻许可分布与 60 日主线可信度分布，避免研究回测再静默回退到 `proxy`。
- [修复] 强势筛选二次决策的组合优先级分现在会把 `NaN / inf` 的前瞻分归一为中性值，避免 `portfolio[].score` 与 `forward_alpha_score` 在页面和回测详情里出现空值。
- [改进] 强势筛选二次决策继续校准同主线收口：当当前主仓仍是 `front/mid/back + waiting`，且同主线已入选组合里存在 `leader + clear` 候选时，会优先把主仓切回更明确的主线核心；次仓选择也会在主仓尚未 clear 时优先补同主线确认票。
- [修复] 修正 V1 强势筛选回测 `regime_breakdown` 对市场环境的口径映射：`medium` 统一并入 `general`，避免摘要中的市场环境分布与分档统计不一致。
- [改进] 校准强势筛选官方 Top3 收口：主仓改为优先选择“次日执行更清晰”的候选，同主线 `mid/back waiting` 被误放主仓时自动让位给 `leader/front` 计划买点更清晰的标的，观察仓同时加强“主线确认票”与高延续前排的优先级。
- [改进] 校准强势筛选二次决策总闸门：当市场环境为 `medium`、机会质量为 `mid`，但核心溢价或广度溢价至少一项仍强、主仓盈亏比过线且组合已有明确计划买点时，上调为“谨慎出手”，并在回测诊断中补充“动作矩阵收口” blocker 解释。
- [修复] 强势筛选 AI 点评请求补齐二次决策新总闸门字段，避免后端校验缺少 `decision.attack_permission` 时返回 422 导致点评无法打开。
- [修复] 提高 Docker 定时分析容器默认内存上限，避免 18:00 大盘复盘与自选股分析并发运行时因 512MiB cgroup OOM 被杀后错过当日任务。
...
- [改进] 强势筛选二次决策校准“买点清晰度”口径：`waiting` 但已生成明日建议区间、风险可控且未明显追高的个股，会计入明日买点计划清晰，减少强市场下被机会质量误伤为仅观察
- [改进] 强势筛选二次决策落地新总闸门运行口径：接口新增 20日进攻许可、60日主线可信度、机会质量矩阵与风险提示条，页面同步切换为双卡表达并隐藏“策略健康/历史有效性”旧心智
- [文档] 新增《新总闸门产品规则草案》，正式收口 20日进攻许可、60日主线可信度、市场环境、机会质量、动作矩阵与停用恢复的新产品规则
- [文档] 更新强势筛选二次决策 SPEC：移除用户侧“策略健康”旧词，并同步新的 20日/60日 分工、市场环境与机会质量主矩阵、以及停用恢复口径
- [文档] 重写《强势筛选 V1 产品原则 + 总闸门规则》中的旧闸门章节，统一对齐新总闸门框架与用户侧命名
- [文档] 更新强势筛选二次决策 PRD：同步新总闸门心智、双卡页面表达、市场环境与机会质量定义以及动作矩阵
- [文档] 更新强势筛选二次决策技术开发文档：补齐 20日进攻许可、60日主线可信度、主矩阵封顶、风险提示条与停用恢复的实现口径
- [文档] 更新强势筛选二次决策开发任务清单：按新总闸门拆解后端规则链、API 冻结顺序、前端双卡切换、风险提示条和停用恢复实施顺序
- [改进] 强势筛选二次决策新增“同主线主仓纠偏”规则：当 `mid/back waiting` 被抬成主仓、且已入选组合里同主线存在 `leader/front clear` 候选时，自动把主仓换回更合理的主线核心
- [改进] V1 强势筛选回测进一步拆分排序诊断：`main_slot_underperformed` 仅保留同主线主仓错排，跨主线但已进入默认组合的强票改记为 `portfolio_anchor_underperformed`，避免把组合层面的黑马表现误判成主仓排序错误
- [改进] V1 强势筛选回测的问题诊断收紧 `main_slot_underperformed` 口径：只有“同主线错排”或“默认组合内已有更强股但主仓没排到前面”时才继续报警，跨主线黑马不再混入主仓排序问题
- [改进] 强势筛选二次决策放松“买点清晰度”对主仓 `clear` 的单点硬门槛：当主仓仍在等待但组合内已有 2 只股票形成明确买点时，机会质量可提升到中性评估，帮助总闸门减少对“仅主仓未 clear”场景的过度观察降级
- [修复] V1 强势筛选回测任务在服务重启后改为按 `processed_trade_dates + failed_trade_dates` 断点续跑，不再把 `running -> queued` 的任务进度清零并从首个交易日整段重放
- [改进] 强势筛选二次决策主链路按 60 日回测诊断进行首轮校准：主仓排序取消 `clear` 硬门槛，角色/买点改为轻量解释修正，弹性分改为非线性的延续潜力信号，并新增 `forward_alpha_score` 与候选股级别排序诊断，方便回测复盘主仓错排
- [修复] V1 强势筛选回测在长区间场景下会识别 `_get_trade_dates` 返回范围不足并回退到 `trade_cal`，同时把回放阶段的策略健康改为“优先复用缓存、否则回退代理健康度”，避免 60 日回测被逐日递归的 20/60 历史验证拖慢或卡死
- [改进] V1 强势筛选回测任务中心新增最近刷新时间、手动刷新反馈和可视化进度条，并在已加载任务详情中同步展示“当前任务快照”，减少任务仍在执行时的误判与盲刷
- [修复] Momentum screener 回测服务改为持有独立的 secondary-decision 实例，不再覆盖页面交互使用的共享异步实例，避免先进入回测后强势筛选退化为同步策略健康重算并触发 504
- [修复] V1 强势筛选回测在 `secondary_decision` 长耗时阶段会持续刷新 `heartbeat_at` 与阶段文案，避免任务中心长时间停在旧时间戳、看起来像已卡死
- [修复] V1 强势筛选回测在后台实际执行交易日列表与任务创建时预估列表不一致时，会在运行启动阶段自动重置 `total_trade_dates / processed_trade_dates / failed_trade_dates`，避免已完成任务出现 `11/12` 这类进度不一致显示
- [修复] Docker Compose 为 `stock-server` 单独提升到 `1G` 内存上限，避免 V1 强势筛选回测在 Web/API 容器内因 `512M` cgroup 限制被 OOM kill 后反复重启
- [改进] V1 强势筛选回测任务中心升级为串行队列：同参任务默认复用，任务列表按“当前运行 / 排队中 / 历史任务”三段展示，并新增刷新进度、取消任务、删除任务、按交易日+阶段显示当前进度的能力
- [改进] 强势筛选页移除“官方明日观察池摘要”卡片与复制入口，页面只保留筛选结果、二次决策、盘中信号和 Aggressive 补充视图，减少无效信息干扰
- [改进] Web 前端全站 API 默认请求超时从 30 秒提升到 180 秒，减少慢接口在客户端过早中断
- [修复] V1 强势筛选回测改为创建后立即返回 run_id 并转入后台计算，页面自动轮询进度，避免创建阶段长期阻塞后前端超时
- [改进] V1 强势筛选回测新增最近任务列表与一键加载入口，run_id 持久化任务可在切换页面后继续回捞和查看进度
- [改进] V1 强势筛选回测继续补齐总闸门诊断：summary 新增总闸门细分模块区间聚合，daily detail 新增当日总闸门快照与拖后腿模块，帮助定位究竟是指数趋势、赚钱效应、市场情绪还是买点清晰度在限制出手
- [改进] 强势筛选 V1 总闸门继续贴近真实市场状态：市场环境优先接入指数 5 日线/3 日斜率与全市场涨跌停统计，页面总闸门子模块改为中文显示，`谨慎出手` 盘中收口新增“仅主仓可考虑”
- [改进] 强势筛选 V1 官方入口固定为 `4 / 2亿 / 2% + Top30`，二次决策与回测统一收口到 Standard 主引擎，Aggressive 只保留折叠式进攻补充观察层
- [???] V1 ?????????????????????? `/backtest` ??? ?V1 ?????? / ????? ?????????? run??? run???????????????????
- [改进] V1 强势筛选回测补齐区间摘要结构与结果页交互：summary 新增比较基准/分层诊断/市场分桶聚合，daily 接口新增日期/市场分桶/总闸门/槽位/主线筛选与分页，`/backtest` 页同步升级为概览、基准、诊断、分桶、问题清单、日级回放六区块布局
- [新功能] 新增 V1 强势筛选回测后端首条闭环：支持创建同步回测 run、冻结日级 screening/decision 结果、回填 T+1/T+2 outcome，并提供 run/summary/daily 查询接口
- [改进] 强势筛选 V1 总闸门补齐剩余规则实现：新增核心赚钱效应分层、主仓买点不清晰降级、主仓/次仓过度加速时仅观察，以及仅在市场/机会/历史/核心赚钱效应同时满足时才升级为 `strong_go`
- [改进] 强势筛选页面收口为 Standard 官方主结论 + Aggressive 进攻补充视图，统一官方观察池、结果列表与详情入口的主辅关系
- [???] ???? V1 ??????????????????????? `5 / 3 / 3`?????????????/???/???????????????????????
- [??] ????????????????????????????????????????? `full / simplified / disabled` ???????????????????
- [??] ?? Momentum Screener V1 ????? API ????????? screening / decision / intraday / AI ???????????
- [文档] 新增强势筛选《V1 开发改造清单》和《V1 回测开发任务清单》，分别固化产品改造顺序与回测系统实施顺序
- [文档] 新增强势筛选《V1 回测数据表与接口草案》，明确 run 级、日级、候选池级、决策级、结果级数据结构与摘要/日级/单日/问题清单接口草案
- [文档] 新增强势筛选《V1 回测数据口径与结果面板设计》，固定回测记账字段、核心指标定义、比较基准与结果页结构
- [文档] 新增强势筛选《V1 回测与问题诊断框架》，固化统一回测基线、五层验证结构、市场分桶、比较基准、失败归因与受控进化闭环
- [文档] 新增强势筛选《V1 产品原则 + 总闸门规则》，固定全市场统一入口 `4 / 2亿 / 2% + Top30`，明确 Standard/Aggressive 职责边界、三层总闸门与五档出手级别执行规则
- [改进] 强势筛选 AI 点评改为“一级规则抽屉 + 二级对话弹层”布局：一级仅展示规则与点评预览，完整 AI 输出、快捷追问和输入区收口到二级弹层，避免头部与底部卡片互相挤压
- [修复] 强势筛选 AI 点评修复盘中解读在未刷新盘中信号时误复用“落选分析”规则结论的问题，并为会话 ID 增加版本因子以避免恢复旧错误会话
- [新功能] 强势筛选页新增 AI 点评侧栏与流式点评接口，支持候选股点评、二次决策综合建议、盘中信号解读与落选分析四类入口，以及信任条、快捷追问和 resume/rerun 会话语义
- [文档] 更新强势筛选 AI 点评 PRD 与技术设计，明确其作为解释层的产品定位，并补齐入口优先级、回答合同、信任条、会话重开策略与盘中风控边界
...
- [改进] 强势筛选二次决策的 `strategy_health` 增加后台预热与磁盘缓存：API 首轮优先返回代理健康度并异步计算真实 20/60 日历史验证结果，算完后复用缓存，减少页面首次请求被长时间阻塞的问题
- [改进] 强势筛选二次决策的 `strategy_health` 升级为真实 20/60 日历史验证结果：按同一套筛选参数回放最近交易日，基于默认组合的 1-2 日利润窗口、回撤与成功率计算健康状态，并在接口与页面同步展示样本数、成功率、平均利润窗口和平均回撤
- [修复] 二次决策页在代理健康度阶段补回可见的“刷新真实 20/60 结果”入口，并让手动刷新显式等待真实历史验证结果返回，避免用户反复刷新后仍长期停留在代理健康度提示

- [新功能] 强势筛选页新增“二次决策”结果区与 `/api/v1/stocks/screener/momentum/decision` 接口，基于现有候选池输出今日出手级别、主线识别、默认 1-3 票组合、落选原因与执行提示
- [新功能] 强势筛选页新增“盘中信号”面板与 `/api/v1/stocks/screener/momentum/decision/intraday` 接口，支持手动刷新主仓/次仓/观察仓的触发状态、低置信度提示、不建议追入与 60 分钟内买/不买收口
- [改进] 强势筛选二次决策补齐 `action_checklist`、`closing_note` 与 `focus_order` 载荷，并在页面新增“明日行动清单”“优先关注顺序”和更明确的 60 分钟收口说明
- [改进] 强势筛选二次决策新增 `strategy_health` 双窗口健康度输出与页面展示，明确“正常 / 谨慎 / 恢复中 / 停用”状态，并让健康度约束今日出手级别与组合执行提示

- [文档] 新增强势筛选二次决策开发任务清单，明确后端、接口、前端、测试的实施顺序与里程碑拆分
- [文档] 新增强势筛选二次决策原始 SPEC、完整需求文档与技术开发文档，统一沉淀主线识别、默认组合、低置信度、价格偏离与盘中决策规则
- [文档] 更新 `docs/architecture/README.md`，补充二次决策文档索引入口，便于后续产品、研发与测试协作检索
- [修复] 强势筛选页改为仅在点击“执行筛选”后运行，并修复结果详情抽屉自动打开且无法关闭的问题
- [修复] 强势筛选个股详情页的维度拆解支持驼峰、下划线和全大写字段名统一翻译，避免直接向用户展示英文原始指标键名
- [新功能] 强势筛选页新增“明日观察池摘要”区块，基于当前筛选结果自动整理 Top3、题材聚焦、风险提醒与可复制的执行建议
- [文档] 补充“明日观察池摘要”MVP 需求文档与 AI 优化记录，便于说明本次最小痛点与作业交付物
- [文档] 补充“明日观察池摘要”飞书表格提交说明，提供可直接粘贴的作业简介、截图建议和源码打包清单
