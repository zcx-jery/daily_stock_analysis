...
| 首页分析 | 扩展字段 + 任务进度 | 首页分析结果支持展示固定狙击点位之外的扩展字段，并为字段映射补充中文解释；分析任务卡片支持打开详细进度抽屉，查看阶段进度与时间线 |
...
> 首页分析现在会把模型输出中未纳入固定字段的扩展点位一并记录下来，并在报告详情中以“中文解释（原始字段名）”形式展示，便于排查不同策略下的字段漂移；固定字段 `ideal_buy / secondary_buy / stop_loss / take_profit` 主链保持不变。

> 首页左侧“分析任务”面板中的 `分析中 / 等待中` 状态现支持点击展开详细进度；当后端回传了阶段事件时，会展示任务创建、阶段推进、完成/失败时间线；若暂时没有事件明细，也会至少展示当前进度与状态。
